    ...
    IMcpClientFactory mcpClientFactory) : IDatabricksQueryBriefingRunner
{
    ...
    public async Task<AIResult> GetBriefing(DatabricksQueryBriefingParameters briefing)
    {
        if (string.IsNullOrEmpty(briefing.Prompt))
        {
        ...
        promptBuilder.AddUserMessage(briefing.Prompt);
